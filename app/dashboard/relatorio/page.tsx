      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:16, marginBottom:28 }}>
        {[
          ...
          <div key={m.label} style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:16, padding:20 }}>
            ...
            <p style={{ fontFamily:'Syne,sans-serif', fontSize:32, fontWeight:800, color:m.color, lineHeight:1 }}>{m.value}</p>
          ...
          <thead>
            <tr style={{ borderBottom:'1px solid var(--border)' }}>
              {['Fase','Leads','% do Total','Dias Médio','Com Feedback'].map(h=>(
                <th key={h} style={{ padding:'10px 0', textAlign:'left', fontSize:10, textTransform:'uppercase', letterSpacing:'0.8px', color:'var(--muted)', fontWeight:600 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {FASES.map(fase=>{
              ...
              return (
                <tr key={fase} style={{ borderBottom:'1px solid rgba(255,255,255,0.03)' }}>
                  ...
                  <td style={{ padding:'14px 0', fontFamily:'Syne,sans-serif', fontWeight:700, color:cor.text, fontSize:18 }}>{cnt}</td>
                  <td style={{ padding:'14px 0' }}>
                  ...
                  <td style={{ padding:'14px 0', fontSize:13 }}>{med}{med!=='—'?'d':''}</td>
